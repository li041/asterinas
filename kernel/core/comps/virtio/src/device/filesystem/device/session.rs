...
impl FuseSession {
    ...
    pub fn read(
        ...
        let waiter = self.read_async(nodeid, read_request, smallvec::smallvec![data_buf], None)?;
    ...
    pub fn readdir(
        ...
        let waiter = self.device.submit_fuse_op(
            nodeid,
            &mut operation,
            Some(FuseDataBuf::Read(smallvec::smallvec![data_buf.clone()])),
            None,
        )?;
        let payload_len = waiter.wait().payload_len()?;
    ...
    pub fn write(
        ...
        let waiter =
            self.write_async(nodeid, write_request, smallvec::smallvec![data_buf], None)?;
        let payload_len = waiter.wait().payload_len()?;
