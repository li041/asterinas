...
impl PageCacheBackend for VirtioFsInode {
    fn read_page_async(
        ...
        let read_offset = page_offset(idx)?;
    ...
    fn read_pages_async(&self, mut pages: PageRun<'_>, io_batch: &mut IoBatch) -> Result<()> {
        ...
        while pages.len() > 0 {
            ...
            let chunk_start = page_index(start_idx, run_offset)?;
            let read_offset = page_offset(chunk_start)?;
    ...
    fn write_page_async(
        ...
        validate_write_page_range(idx, 1, file_size)?;

        let handle = self.writable_page_handle()?;
        let nodeid = self.nodeid();
        let session = self.fs_ref().session().clone();
        let write_offset = page_offset(idx).unwrap();
    ...
    fn write_pages_async(&self, mut pages: PageRun<'_>, io_batch: &mut IoBatch) -> Result<()> {
        let file_size = self.size();
        let start_idx = pages.start_idx();
        validate_write_page_range(start_idx, pages.len(), file_size)?;
        ...
        while pages.len() > 0 {
            ...
            let chunk_start = page_index(start_idx, run_offset).unwrap();
            let chunk_offset = page_offset(chunk_start).unwrap();
            ...
            for (chunk_pos, locked_page) in (&mut pages).enumerate().take(chunk_len) {
                let page_idx = page_index(chunk_start, chunk_pos).unwrap();
                let page_start = page_offset(page_idx).unwrap();
...
impl ReadChunk {
    ...
    fn push(&mut self, locked_page: LockedCachePage) -> Result<()> {
        let total_len = self.total_len + PAGE_SIZE;
...
fn validate_write_page_range(start_idx: usize, page_count: usize, file_size: usize) -> Result<()> {
    if page_count == 0 {
        return Ok(());
    }

    let last_idx = page_index(start_idx, page_count - 1)?;
    let last_offset = page_offset(last_idx)?;
...
fn page_index(start_idx: usize, offset: usize) -> Result<usize> {
    start_idx
        .checked_add(offset)
