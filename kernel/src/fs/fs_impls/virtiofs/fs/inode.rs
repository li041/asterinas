...
use aster_fuse::{FuseAttrOut, OpenFlags, SetattrIn, SetattrValid};
use aster_virtio::device::filesystem::device::VirtioFsDirEntry;
use ostd::{
...
use super::{
    ...
    handle::{VirtioFsDirHandle, VirtioFsHandle},
    valid_duration,
};
use crate::{
    fs::{
...
impl VirtioFsInode {
    ...
    fn revalidate_lookup(&self, parent_nodeid: u64, name: &str) -> Result<()> {
        ...
        let fs = self.fs_ref();
        let entry_out = fs.device.fuse_lookup(parent_nodeid, name)?;
        ...
        if entry_out.nodeid != old_nodeid {
            ...
            fs.device.fuse_forget(entry_out.nodeid, 1)?;
    ...
    pub(super) fn revalidate_attr(&self) -> Result<()> {
        ...
        let fs = self.fs_ref();
        let attr_out = fs.device.fuse_getattr(self.nodeid())?;
    ...
    fn open_handle(&self, access_mode: AccessMode) -> Result<VirtioFsHandle> {
        ...
        let fs = self.fs_ref();
        let open_out = fs.device.fuse_open(self.nodeid(), flags)?;
        let cache_enabled =
        ...
        let Some(inode) = self.this.upgrade() else {
            fs.device.fuse_release(self.nodeid(), open_out.fh, flags)?;
    ...
    fn open_dir_handle(&self) -> Result<VirtioFsDirHandle> {
        let fs = self.fs_ref();
        let fh = fs.device.fuse_opendir(self.nodeid())?;

        let Some(inode) = self.this.upgrade() else {
            fs.device.fuse_releasedir(self.nodeid(), fh)?;
            return_errno_with_message!(Errno::EIO, "virtiofs inode is unavailable");
        };
    ...
    fn setattr(&self, setattr_in: SetattrIn) -> Result<()> {
        let fs = self.fs_ref();
        let attr_out = fs.device.fuse_setattr(self.nodeid(), setattr_in)?;
...
impl Inode for VirtioFsInode {
    ...
    fn create(&self, name: &str, type_: InodeType, mode: InodeMode) -> Result<Arc<dyn Inode>> {
        ...
        let (entry_out, open_out_opt) = match type_ {
            InodeType::File => {
                let (entry_out, open_out) = fs.device.fuse_create(
                    parent_nodeid,
                    name,
            ...
            InodeType::Dir => {
                let entry_out = fs.device.fuse_mkdir(
                    parent_nodeid,
                    name,
            ...
            InodeType::Socket => {
                let entry_out = fs.device.fuse_mknod(
                    parent_nodeid,
                    name,
        ...
        let attr_out: FuseAttrOut = fs.device.fuse_getattr(entry_out.nodeid)?;
    ...
    fn link(&self, old: &Arc<dyn Inode>, name: &str) -> Result<()> {
        ...
        let fs = self.fs_ref();
        fs.device.fuse_link(old.nodeid(), self.nodeid(), name)?;
        Ok(())
    }
    ...
    fn unlink(&self, name: &str) -> Result<()> {
        ...
        let fs = self.fs_ref();
        fs.device.fuse_unlink(self.nodeid(), name)?;
        Ok(())
    }
    ...
    fn rmdir(&self, name: &str) -> Result<()> {
        ...
        let fs = self.fs_ref();
        fs.device.fuse_rmdir(self.nodeid(), name)?;
        Ok(())
    }
    ...
    fn read_link(&self) -> Result<SymbolicLink> {
        ...
        let fs = self.fs_ref();
        let target = fs.device.fuse_readlink(self.nodeid())?;
