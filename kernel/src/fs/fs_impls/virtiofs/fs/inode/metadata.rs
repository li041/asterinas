...
use aster_fuse::{Attr, FuseInitFlags, SetattrIn, SetattrValid};
...
impl VirtioFsInode {
    pub(in super::super) fn commit_metadata_changing_reply(
        ...
        if should_invalidate && let Some(page_cache) = &self.page_cache {
            let new_size = self.size();
            if new_size != old_size {
                page_cache.resize(new_size, old_size)?;
            } else if session_flags.contains(FuseInitFlags::AUTO_INVAL_DATA)
                && self.mtime() != old_mtime
            {
    ...
    pub(in super::super) fn commit_local_write(
        &self,
        new_size: usize,
        old_size: usize,
    ) -> Result<()> {
