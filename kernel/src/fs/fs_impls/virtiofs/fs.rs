...
impl VirtioFs {
    fn new(device: Arc<FileSystemDevice>, tag: String) -> Result<Arc<Self>> {
        ...
        let root_entry = conn.lookup(FUSE_ROOT_ID, ".")?;
        ...
        let attr_valid_until =
            valid_until(now, root_entry.attr_valid(), root_entry.attr_valid_nsec());
