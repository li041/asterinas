...
use aster_virtio::device::filesystem::device::{FileSystemDevice, get_device_by_tag};
...
impl VirtioFs {
    fn new(device: Arc<FileSystemDevice>, tag: String) -> Result<Arc<Self>> {
        ...
        let fuse_attr_out = device.fuse_getattr(FUSE_ROOT_ID)?;
        let root_metadata = super::metadata_from_attr(fuse_attr_out.attr, container_dev_id);
