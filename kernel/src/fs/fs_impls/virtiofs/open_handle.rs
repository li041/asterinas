...
/// A transient FUSE open handle.
pub(super) struct TransientHandle {
    handle: Arc<VirtioFsOpenHandle>,
}

impl TransientHandle {
    /// Creates a transient handle from a server-issued open handle.
    pub(super) fn new(handle: Arc<VirtioFsOpenHandle>) -> Self {
        Self { handle }
    }
}

impl Deref for TransientHandle {
    type Target = VirtioFsOpenHandle;
