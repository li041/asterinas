...
use aster_virtio::device::transport9p::{
    ...
    protocol::{P9_GETATTR_ALL, P9_NOFID, P9Attr},
};
...
impl Virtio9P {
    fn new(device: Arc<Transport9PDevice>, tag: String) -> Result<Arc<Self>> {
        ...
        Ok(Arc::new_cyclic(|weak_fs| {
            let root = Virtio9PInode::new(root_fid, root_qid, root_metadata, weak_fs.clone());
...
pub(super) fn p9_attr_to_metadata(attr: &P9Attr) -> Metadata {
    ...
    Metadata {
        ...
        blk_size: if attr.blksize > 0 {
            attr.blksize as usize
        } else {
            4096
        },
