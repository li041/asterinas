...
fn get_siginfo_from_user(info_ptr: Vaddr, sig_num: SigNum, ctx: &Context) -> Result<siginfo_t> {
    if info_ptr != 0 {
        let si = ctx
            .user_space()
            .read_val::<siginfo_t>(info_ptr)
            .map_err(|_| {
                Error::with_message(Errno::EFAULT, "failed to read siginfo from user space")
            })?;
