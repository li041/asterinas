...
impl Vmo {
    ...
    pub fn read(&self, offset: usize, writer: &mut VmWriter) -> Result<()> {
        ...
        let Some(read_end) = offset.checked_add(read_len) else {
            return_errno_with_message!(Errno::EINVAL, "read range overflows");
        };
    ...
    pub fn write(&self, offset: usize, reader: &mut VmReader) -> Result<()> {
        ...
        let Some(write_end) = offset.checked_add(write_len) else {
            return_errno_with_message!(Errno::EINVAL, "write range overflows");
        };
    ...
    pub(super) fn fill_zeros(&self, range: Range<usize>) -> Result<()> {
        ...
        if range.end > self.size() {
            return_errno_with_message!(Errno::EINVAL, "filled range exceeds the vmo size");
        }
...
impl<'a> BackedVmo<'a> {
    ...
    pub(super) fn evict_up_to_date_pages(&self, range: &Range<usize>) -> Result<()> {
        ...
        self.wait_for_writeback_and_remove_pages(pages_to_evict, |idx, page| {
            idx >= npages || page.is_up_to_date()
        });
    ...
    pub(super) fn decommit_pages(
        ...
        self.wait_for_writeback_and_remove_pages(pages_to_decommit, |_, _| true);
    ...
    fn wait_for_writeback_and_remove_pages<F>(
        &self,
        ...
        F: FnMut(usize, &CachePage) -> bool,
    {
        ...
        for (page_idx, page) in pages_to_remove {
            let mut cursor = locked_pages.cursor_mut(page_idx as u64);
            let Some(current_page) = cursor.load() else {
                continue;
            };

            if current_page.paddr() == page.paddr() && should_remove(page_idx, &current_page) {
                cursor.remove();
            }
        }
    }
