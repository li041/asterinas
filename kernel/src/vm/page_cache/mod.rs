...
//!   to make a page readable or durable. [`BlockBackend`] is a helper interface
//!   for backends that can satisfy that contract with block-device BIOs.
//!
...
/// devices usually implement [`BlockBackend`] instead.
...
/// A block-device submission interface for page-cache backends.
///
...
pub trait BlockBackend: Sync + Send {
...
impl<T: BlockBackend> PageCacheBackend for T {
    fn read_page_async(
        &self,
        idx: usize,
        locked_page: cache_page::LockedCachePage,
