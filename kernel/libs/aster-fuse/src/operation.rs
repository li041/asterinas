...
pub trait FuseOperation {
    ...
    /// the `OutHeader`. Implementations must use `payload_len` to bound the
    /// bytes accepted from the daemon.
    fn parse_reply(
        payload_len: usize,
...
pub enum ReplyExpectation {
    ...
    None,
    /// A reply contains only an `OutHeader` and no payload bytes.
    HeaderOnly,
    /// A reply contains an `OutHeader` and a payload up to the given payload bytes.
