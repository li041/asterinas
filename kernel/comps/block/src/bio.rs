...
/// After submission, that ownership is transferred to `SubmittedBio`, while the
/// `IoBatch` used in submission keeps shared metadata for waiting and status queries.
pub struct Bio {
...
impl Bio {
    ...
    pub fn submit_and_wait(
        ...
        let metadata = io_batch
            .get(0)
            .unwrap()
            .downcast_ref::<BioMetadata>()
            .unwrap();
        Ok(metadata.status())
    }
}
