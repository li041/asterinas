...
//!
//! [`FuseConnection`] is mount-scoped FUSE session. It performs
//! `FUSE_INIT` negotiation, and exposes typed request helpers.

use alloc::{string::String, sync::Arc, vec::Vec};
use core::sync::atomic::{AtomicU64, Ordering};

use aster_fuse::{ops::release::ReleaseFlags, *};
use ostd::{
    info,
...
impl AttrVersion {
    /// The version of a newly created inode before any attribute commit.
...
impl FuseConnection {
    ...
    pub fn current_attr_version(&self) -> AttrVersion {
...
impl FuseConnection {
    ...
    pub fn fuse_forget(&self, nodeid: FuseNodeId, nlookup: u64) {
    ...
    pub fn fuse_readlink(&self, nodeid: FuseNodeId) -> Result<String, FuseError> {
    ...
    pub fn fuse_unlink(&self, parent_nodeid: FuseNodeId, name: &str) -> Result<(), FuseError> {
        self.device
            .do_fuse_op(parent_nodeid, UnlinkOperation::new(name))
    }

    pub fn fuse_rmdir(&self, parent_nodeid: FuseNodeId, name: &str) -> Result<(), FuseError> {
        self.device
    ...
    pub fn fuse_open(&self, nodeid: FuseNodeId, flags: u32) -> Result<OpenOut, FuseError> {
        self.device
    ...
    pub fn fuse_opendir(&self, nodeid: FuseNodeId) -> Result<OpenOut, FuseError> {
        self.device
