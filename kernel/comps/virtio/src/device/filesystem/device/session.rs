...
use ostd::{
    info,
    mm::{USegment, io::util::HasVmReaderWriter},
};
...
impl FuseSession {
    ...
    pub fn bump_attr_version(&self) -> AttrVersion {
        // Use the strongest ordering to give concurrent attribute-version bumps
        // a single global order before callers compare versions.
        // FIXME: Handle potential overflow of the version counter, which may cause stale.
        AttrVersion::new(self.attr_version.fetch_add(1, Ordering::SeqCst) + 1)
    }
    ...
    /// Maps `segment` as a DMA buffer for `FUSE_READ` data.
    pub fn new_map_read_buf(&self, segment: USegment) -> Result<FuseReplyBuf, FuseError> {
        FuseReplyBuf::new_map(segment).map_err(FuseError::ResourceAlloc)
    }
    ...
    /// daemon replies in a non-sleepable completion context.
    pub fn read_async(
        &self,
        nodeid: FuseNodeId,
        read_request: ReadIn,
    ...
    ///
    /// Returns request submission errors and daemon-reported release errors.
