...
use aster_fuse::{FuseCompleteFn, FuseCompletion, FuseNodeId, FuseUnique, OutHeader};
use ostd::mm::io::util::HasVmReaderWriter;
use smallvec::SmallVec;

use super::waiter::{FuseWaiter, ReplyBufs};
use crate::device::filesystem::pool::RequestDmaBuf;

/// DMA buffers sent to the device as one FUSE request.
pub(super) type RequestBufs = SmallVec<[RequestDmaBuf; 2]>;
...
impl FuseRequest {
    ...
    pub(super) fn request_bufs(&self) -> &[RequestDmaBuf] {
    ...
    fn parse_reply_header(&self, reply_len: usize) -> FuseCompletion {
        ...
        if reply_len < size_of::<OutHeader>() {
            return FuseCompletion::MalformedResponse;
        }
        for reply_buf in reply_bufs {
            reply_buf
                .mem_obj()
                .sync_from_device(reply_buf.offset().clone())
                .unwrap();
        }

        let Some(reply_header_buf) = reply_bufs.first() else {
            return FuseCompletion::MalformedResponse;
        };

        let mut reader = reply_header_buf.reader().unwrap();
        let Ok(reply_header) = reader.read_val::<OutHeader>() else {
            return FuseCompletion::MalformedResponse;
        };
        ...
        if let Some(payload_len) = (reply_header.len() as usize).checked_sub(size_of::<OutHeader>())
        {
