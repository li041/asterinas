...
//!
//! This module defines [`FileSystemDevice`] and the request-side helpers used to
//! send FUSE operations over virtiofs queues. The `client` submodule provides
//! high-level FUSE methods, `helpers` prepares DMA-backed request buffers, and
//! `virtio_ops` drives queue submission and completion handling.
//!
...
use aster_fuse::*;
pub use connection::FuseConnection;
use operation::ForgetOperation;
use ostd::{
    arch::trap::TrapFrame,
    debug, info,
    mm::{
...
/// Represents a virtiofs device that issues FUSE requests to a backend server.
///
/// This is the device-scoped layer: it owns the virtio transport, queues, and
/// DMA pools. FUSE protocol state (negotiated caps, max_write, etc.) lives in
/// [`FuseConnection`], which is mount-scoped.
pub struct FileSystemDevice {
    ...
    // Start request IDs at 1 and keep 0 unused. In Linux FUSE,
    // `unique == 0` is reserved for unsolicited notification messages
    // rather than ordinary request/reply matching.
    // Reference: https://codebrowser.dev/linux/linux/fs/fuse/dev.c.html#2200
...
impl FileSystemDevice {
    pub(crate) fn do_fuse_op<Op: FuseOperation>(
        ...
        let request = Arc::new(self.prepare_request(&operation)?);
    ...
    /// `FUSE_FORGET`.
    pub(crate) fn forget(&self, nodeid: u64, nlookup: u64) {
        if nodeid == FUSE_ROOT_ID || nlookup == 0 {
            return;
        }
        let operation = ForgetOperation::new(nodeid, ForgetIn::new(nlookup));
        let request = match self.prepare_request(&operation) {
            Ok(request) => request,
            Err(err) => {
                warn!(
        ...
        if let Err(err) = self.submit(&self.hiprio_queue, Arc::new(request)) {
            warn!(
                "virtiofs forget failed to enqueue inode {} with nlookup {}: {:?}",
                nodeid, nlookup, err
            );
        }
    }
    ...
    pub(super) fn prepare_request<Op: FuseOperation>(
        ...
        let in_buf = self.prepare_in_buf(operation, unique)?;
    ...
    fn select_request_queue(&self, nodeid: u64) -> &FsRequestQueue {
...
/// Represents a directory entry returned by `fuse_readdir`.
#[derive(Debug, Clone)]
...
impl FuseRequest {
    pub(super) fn read_reply<Op: FuseOperation>(
        ...
        let mut out_header = OutHeader::empty();
        out_buf
            .read_bytes(0, out_header.as_mut_bytes())
            .map_err(|_| VirtioDeviceError::QueueUnknownError)?;
        ...
        operation
            .parse_reply(payload_len, &mut |offset, dst_buf| {
                out_buf
                    .read_bytes(size_of::<OutHeader>() + offset, dst_buf)
