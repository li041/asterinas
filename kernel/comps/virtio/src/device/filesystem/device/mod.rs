...
use aster_fuse::{
    FuseCompleteFn, FuseError, FuseNodeId, FuseOperation, FuseUnique, InHeader, OutHeader,
    ReplyExpectation,
};
use ostd::{
    mm::{
...
use spin::Once;
use waiter::FuseWaiter;

pub use self::session::{AttrVersion, FuseSession};
use crate::{
    device::filesystem::pool::{FsDmaPool, FuseDataBuf, FuseReplyBuf, ReplyDmaBuf, RequestDmaBuf},
    dma_buf::DmaBuf,
...
impl FileSystemDevice {
    ...
    fn prepare_request<Op: FuseOperation>(
        ...
        let data_buf_len = match data_buf.as_ref() {
            Some(FuseDataBuf::Write(data_buf)) => DmaBuf::len(data_buf.inner().as_ref()),
            _ => 0,
        };
        ...
        let (request_bufs, reply_bufs) = match data_buf {
            ...
            Some(FuseDataBuf::Write(data_buf)) => {
                let data_buf = data_buf.inner().clone();
                data_buf
                    .mem_obj()
    ...
    fn alloc_and_fill_request_buf(
        ...
        unique: FuseUnique,
        data_buf_len: usize,
    ) -> Result<RequestDmaBuf, FuseError> {
        let request_buf_len = size_of::<InHeader>() + operation.body_len();
        ...
        let request_buf = self
            .to_device_pool
            .alloc_request_buf(request_buf_len)
            .map_err(FuseError::ResourceAlloc)?;
        let request_buf = request_buf.inner().clone();

        let request_header =
            InHeader::new(total_len as u32, operation.opcode() as u32, unique, nodeid);
        ...
        request_buf
            .mem_obj()
            .sync_to_device(request_buf.offset().clone())
            .unwrap();
    ...
    ) -> Result<SmallVec<[ReplyDmaBuf; 2]>, FuseError> {
        match (reply_expectation, data_buf) {
            (ReplyExpectation::None, None) => Ok(smallvec![]),
            (ReplyExpectation::HeaderOnly, None) => Ok(smallvec![self.alloc_reply_buf(0)?]),
            (ReplyExpectation::Payload(payload_size), None) => self
                .alloc_reply_buf(payload_size.get())
                .map(|buf| smallvec![buf]),
            (ReplyExpectation::Payload(payload_size), Some(data_buf)) => {
                if payload_size.get() > DmaBuf::len(data_buf.inner().as_ref()) {
                    return Err(FuseError::BufferTooSmall);
                }

                Ok(smallvec![
                    self.alloc_reply_buf(0)?,
                    data_buf.inner().clone()
                ])
            }
    ...
    fn alloc_reply_buf(&self, payload_size: usize) -> Result<ReplyDmaBuf, FuseError> {
        let total_len = size_of::<OutHeader>()
            .checked_add(payload_size)
            .ok_or(FuseError::LengthOverflow)?;
        self.from_device_pool
            .alloc_reply_buf(total_len)
