...
bitflags::bitflags! {
    /// Represents the virtio-fs feature bits supported by the driver.
...
/// Represents the virtio-fs device configuration layout.
#[repr(C)]
...
impl ConfigManager<VirtioFsConfig> {
    pub(super) fn read_config(&self) -> Result<VirtioFsConfig, VirtioDeviceError> {
        let mut config = VirtioFsConfig::new_zeroed();

        for (index, byte) in config.tag.iter_mut().enumerate() {
            *byte = self.read_once::<u8>(offset_of!(VirtioFsConfig, tag) + index)?;
        }
        config.num_request_queues =
            self.read_once::<u32>(offset_of!(VirtioFsConfig, num_request_queues))?;
