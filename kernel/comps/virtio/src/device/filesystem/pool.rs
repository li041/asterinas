...
type FsDmaBuf<D> = Arc<Slice<FsDmaStorage<D>>>;

/// A DMA buffer for data sent to the virtio-fs device.
pub(crate) type RequestDmaBuf = FsDmaBuf<ToDevice>;

/// A DMA buffer for data received from the virtio-fs device.
pub(crate) type ReplyDmaBuf = FsDmaBuf<FromDevice>;

/// A size-classed allocator for virtio-fs DMA buffers.
#[derive(Debug)]
pub(super) struct FsDmaPool<D: DmaDirection> {
...
impl<D: DmaDirection> FsDmaPool<D> {
    ...
    fn alloc_fs_buf(&self, len: usize) -> Result<FsDmaBuf<D>> {
        debug_assert!(
            len > 0,
            "zero-length buffers should be handled by the caller"
        );
...
impl FsDmaPool<FromDevice> {
    ...
    pub(super) fn alloc_reply_buf(&self, len: usize) -> Result<FuseReplyBuf> {
        self.alloc_fs_buf(len).map(FuseReplyBuf)
    }
}
...
impl FsDmaPool<ToDevice> {
    ...
    pub(super) fn alloc_request_buf(&self, len: usize) -> Result<FuseRequestBuf> {
        self.alloc_fs_buf(len).map(FuseRequestBuf)
    }
}
...
#[derive(Clone, Debug)]
pub struct FuseRequestBuf(RequestDmaBuf);

impl FuseRequestBuf {
    /// Returns the underlying request DMA buffer.
    pub(crate) fn inner(&self) -> &RequestDmaBuf {
        &self.0
    }
}
...
#[derive(Clone, Debug)]
pub struct FuseReplyBuf(ReplyDmaBuf);

impl FuseReplyBuf {
    /// Maps `segment` as a DMA buffer for FUSE reply payloads.
    pub(super) fn new_map(segment: USegment) -> Result<Self> {
    ...
    /// Returns the underlying reply DMA buffer.
    pub(crate) fn inner(&self) -> &ReplyDmaBuf {
        &self.0
    }
}
