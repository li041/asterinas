...
FN_SETUP(create_process)
{
	...
	process_pidfd = CHECK(open(path, O_RDONLY | O_CLOEXEC));

	// Verify that lseek works on the pidfd
	CHECK_WITH(lseek(process_pidfd, 42, SEEK_SET), _ret == 42);
...
// TODO: Enable thread tests once pidfd for threads is supported
// FN_SETUP(create_thread)
// {
// 	CHECK(pthread_create(&thread, NULL, thread_func, NULL));
// 	usleep(100);

// 	printf("thread_tid=%d, pid=%d\n", thread_tid, getpid());
